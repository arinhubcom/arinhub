...
- `line`: The specific line number in the new version of the file (must be within the diff hunk)
- `body`: A concise, actionable comment explaining the issue and suggesting a fix
...
- Suggest a fix when possible using code blocks
...
- For multi-line issues, use the `start_line` and `line` fields together to highlight a range
